...
function Checkout({ cart }) {
  ...
  const getTotalPrice = () => {
    const totalPrice = cart.reduce((total, item) => total + item.price * item.quantity, 0); // calculates the total price of all items
  ...
  const handleConfirmOrder = async (e) => {  //order c
    e.preventDefault();
